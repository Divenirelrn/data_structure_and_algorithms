...
线段树：
	对于一个数组，查找某段（从下标i到下标j）的和(query)与更新数组(update)的时间复杂度分别为o(n)与o(1)
	但要将query的时间复杂度降为o(1)(利用前缀和)，那么update的时间复杂度又会变为o(n)
	可见query与update的时间复杂度相互牵制，线段树是使得两者的时间复杂度均为o(logn)的一种数据结构
	...
	线段树示例：
	 ...
	 其中，N表示None，[0-5]表示arr[0:6]的和
	 可见，线段树是一棵完全二叉树
	 
	 线段树的存储结构为数组，数组下标对应于线段树从上至下、从左至右的下标（从0开始）
	 则一个节点的左右孩子下标为：
	 left_node = 2 * node + 1
	 right_node = 2 * node + 2
